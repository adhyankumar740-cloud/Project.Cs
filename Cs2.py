            ...
            for room_record in room_statuses:
                ...
                if room_record[0] == room_num_input:
                    ...
                    if room_record[1].upper() == 'VACANT': 
                        co.execute("insert into roombooking values({},{},{},'{}','{}',{},'{}','{}','{}')".format(room_num_input, booking_id, 0, user_name, address, phone_num, check_in_date, check_out_date, gender))
...
Please choose another room. ''')
    ...
    if restaurant_choice == 1:
        ...
        co.execute('insert into order_info values({},{},{},{})'.format(order_num, 0, table_num, total_amount))
...
elif main_choice == 4: # New User Registration Logic
    print('-------------------------------------------------')
    ...
    try:
        ...
        # Insert new user record. Modify this if the 'user_info' schema is different.
        # Assuming table structure: (user_id, user_name, password, full_name, address, phone_num, email)
        insert_query = "INSERT INTO user_info VALUES ({}, '{}', '{}', '{}', '{}', {}, '{}')".format(
            user_id, user_name, password, full_name, address, phone_num, email
        )
        co.execute(insert_query)
        
        mydb.commit()
        print("User Registered Successfully! Your User ID is:", user_id)
        
    except sqltr.Error as err:
        print(f"Error during registration: {err}")
        print("Please ensure the 'user_info' table exists in the 'taj_hotel' database with the required columns.")
